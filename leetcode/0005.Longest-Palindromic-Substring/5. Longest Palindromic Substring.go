...
func longestPalindrome(s string) string {
	...
	for i := 0; i < len(newS); i++ {
		if i < maxRight {
			...
			dp[i] = min(maxRight-i, dp[2*center-i])// 利用这步可以跳过一些不用检测的位置.加速下面的中心扩散算法.// 这个挺显然的,画图即可. 因为只有在maxRight的左边部分才能保证对称性, dp[2*center-i] 是对称点的扩散值.他俩取min当然就是dp[i]的一个下界!
		}
		// 中心扩散法更新 dp[i]  //经过上面跳过一些搜索点之后继续拓展.
