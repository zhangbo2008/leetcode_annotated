...
// SegmentTree define
type SegmentTree struct {  //线段树的教学视频可以看慕课网liuyubobo的课程.
	data, tree, lazy []int      //data存原始数据, tree存区间值. lazy是懒惰更新
	left, right      int
	merge            func(i, j int) int //merge是区间合并的函数.
}
...
func (st *SegmentTree) Init(nums []int, oper func(i, j int) int) {
	...
	//tree和lazy都是4倍.维护区间信息
	for i := 0; i < len(nums); i++ {
		data[i] = nums[i]
	}//只是简单的把nums数据放到data里面
	st.data, st.tree, st.lazy = data, tree, lazy//写2遍的赋值有意思吗?干啥不直接写
	if len(nums) > 0 {
...
// 在 treeIndex 的位置创建 [left....right] 区间的线段树,建立一个表示区间left到right的区间树,然后这个树
//的根节点放到tree这个数组的treeIndex索引位置. 也就是说虽然是segmentTree但是我们使用数组来表示.
func (st *SegmentTree) buildSegmentTree(treeIndex, left, right int) {
	if left == right {
		st.tree[treeIndex] = st.data[left]
		return
	}
	midTreeIndex, leftTreeIndex, rightTreeIndex := left+(right-left)>>1, st.leftChild(treeIndex), st.rightChild(treeIndex)//先递归建立左右子树
	st.buildSegmentTree(leftTreeIndex, left, midTreeIndex)
	st.buildSegmentTree(rightTreeIndex, midTreeIndex+1, right)
	st.tree[treeIndex] = st.merge(st.tree[leftTreeIndex], st.tree[rightTreeIndex])//然后调用merge建立本身.
}
...
func (st *SegmentTree) queryLazyInTree(treeIndex, left, right, queryLeft, queryRight int) int {
	...
	if st.lazy[treeIndex] != 0 { // this node is lazy// 这个节点有lazy数据就需要lazy数据跟tree数据进行合并结果才返回.
		for i := 0; i < right-left+1; i++ {
			st.tree[treeIndex] = st.merge(st.tree[treeIndex], st.lazy[treeIndex])
			// st.tree[treeIndex] += (right - left + leetcode1) * st.lazy[treeIndex] // normalize current node by removing lazinesss
		}
...
// 以 treeIndex 为根，更新 index 位置上的值为 val// 非lazy只能每次更新一个节点的值.
...
func (st *SegmentTree) updateLazyInTree(treeIndex, left, right, updateLeft, updateRight, val int) {
	...
	if st.lazy[treeIndex] != 0 { // this node is lazy
		for i := 0; i < right-left+1; i++ {
			st.tree[treeIndex] = st.merge(st.tree[treeIndex], st.lazy[treeIndex])
			//st.tree[treeIndex] += (right - left + leetcode1) * st.lazy[treeIndex] // normalize current node by removing laziness
		}
		if left != right { // update lazy[] for children nodes
			st.lazy[leftTreeIndex] = st.merge(st.lazy[leftTreeIndex], st.lazy[treeIndex])
			st.lazy[rightTreeIndex] = st.merge(st.lazy[rightTreeIndex], st.lazy[treeIndex])
